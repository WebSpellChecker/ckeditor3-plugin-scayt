...
(function()
{
	...
	var onEngineLoad = function()
	{
		...
		var createInstance = function()	// Create new instance every time Document is created.
		{
			...
			console.log(oParams.srcNodeRef);
			// syntax : AppName.AppVersion@AppRevision
			oParams.assocApp  = 'CKEDITOR.' + CKEDITOR.version + '@' + CKEDITOR.revision;
			oParams.hostType  = 'CKEDITOR.' + CKEDITOR.version + '@' + CKEDITOR.revision;
			...
			var configs = config.scaytOption || {};
			...
			var scayt_control = new window.scayt( oParams );
				if(typeof configs == "object"){
					for(var key in configs){
						scayt_control.option(key, configs[key]);
					}
				}
		...
		editor.on( 'beforeCommandExec', function( ev )		// Disable SCAYT before Source command execution.
			{
				if ( ( ev.data.name == 'source' || ev.data.name == 'newpage' || ev.data.name == 'templates') && editor.mode == 'wysiwyg' )
				{
